...
Take the intended candidate changes from review to an open pull request. One Codex agent owns the entire workflow.
...
Review the final `git diff` and `git status --short`. After confirming that every visible file belongs to the reviewed candidate and no unsafe file is present, run exactly `git add .` from the repository root. Commit with a concise message that describes the full worktree.
...
- Use the existing narrow approvals for `git add`, `git commit`, `git push`, and `gh pr`.
- Keep approved commands simple; do not wrap them in shell pipelines, substitutions, or environment assignments.
- If a required approval is missing, request that same narrow prefix once and continue.
- Never request a broad Git or shell approval.
- Pause only for unsafe files, missing authority, authentication/permission failure, or unresolved review/verification blockers.
...
- whether `git add .` staged the reviewed whole worktree;
