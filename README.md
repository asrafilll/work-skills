...
| **instagram-sketchnote-carousel** | Creates concise educational Instagram carousels as separate 1080x1350 editorial sketchnote images with a consistent hand-drawn system and visual QA. |
...
| **production-readiness-audit** | Principal Infrastructure / SRE mode: a Production Readiness Review focused on **operational posture** — delivery & rollback, reliability & SLOs, scaling & capacity, data safety & DR, platform & networking, observability. Grounded in SRE PRR + AWS Well-Architected. Graded scorecard + severity-ranked findings + remediation plan; assessment only. Hands app-code security to `security-audit`, frontend out of scope. |
...
| **security-audit** | Principal Security Engineer mode: deep, attacker-minded security audit of an existing repo + production setup. OWASP Top 10 2025 (incl. supply chain, exceptional-condition handling), API Top 10, LLM Top 10; tool sweep (gitleaks/semgrep/osv/trivy) + five manual checklists. Severity-ranked findings with exploit scenarios + remediation plan; assessment only. |
